...
for(targ in cargs){
	if((targ == "--help") | (targ == "-h") | (targ == "/?")){
		print("Apply a random forest to allele data.")
		...
		print("The fourth argument is the minimum number of UMI families to accept.")
...
origTab = filter(read_tsv(origTab), UMI_Count >= minReads)
